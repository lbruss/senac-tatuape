...
Os servidores são a base da infraestrutura de TI das empresas, responsáveis por armazenar dados, executar sistemas corporativos, hospedar sites, bancos de dados e conectar usuários e serviços na rede. Nesse cenário, o Linux, principalmente distribuições como Debian, Ubuntu e Red Hat Enterprise Linux, é amplamente utilizado por oferecer estabilidade, segurança e desempenho.

O aprendizado dos comandos Linux e da administração via terminal é essencial para profissionais de infraestrutura, redes, servidores e segurança da informação. Ferramentas como `hostname`, `hostnamectl` e `systemctl` permitem controlar e administrar serviços do sistema, enquanto o SSH possibilita o acesso remoto seguro aos servidores de qualquer lugar da rede ou internet.

Durante as configurações realizadas, foram montados ambientes completos utilizando:
...
Esses componentes formam a base da maioria dos sistemas web profissionais modernos. O Apache atua como servidor web, o PHP processa aplicações dinâmicas, o MariaDB/MySQL armazena os dados e o WordPress facilita a criação de sites. Já no ambiente Java, o Tomcat executa aplicações `.WAR`, enquanto o MySQL gerencia os bancos de dados.

Também foram realizadas tarefas importantes de administração de banco de dados, como:

* criação de usuários
* acesso remoto
* backup e restore
* gerenciamento gráfico com Workbench

Em resumo, foi construído um ambiente corporativo completo de servidores Linux, capaz de hospedar aplicações web, sites profissionais, sistemas Java e bancos de dados, exatamente como acontece em muitas empresas atualmente. 
